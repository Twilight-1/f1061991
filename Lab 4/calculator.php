...
$result = null; // Переменная для хранения результата

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Фильтруем входные данные
    $num1 = filter_var(trim($_POST['num1']), FILTER_VALIDATE_FLOAT);
    $num2 = filter_var(trim($_POST['num2']), FILTER_VALIDATE_FLOAT);
    $operator = $_POST['operator'];

    if ($num1 !== false && $num2 !== false) {
        // Выполняем соответствующую математическую операцию в зависимости от оператора
        switch ($operator) {
            case '+':
                $result = $num1 + $num2;
                break;
...
*/
if ($result !== null) {
    echo "<h2>Результат: " . htmlspecialchars((string)$result) . "</h2>";
}
?>
